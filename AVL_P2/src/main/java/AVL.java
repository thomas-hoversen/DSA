...
public class AVL<T extends Comparable<? super T>> {
    ...
    private AVLNode<T> helpAdd(AVLNode<T> curr, T data) {
        ...
        if (curr == null) {
            size++;
            return newNode;
        } else if (newNode.getData().compareTo(curr.getData()) < 0) { // data smaller than curr.data
            curr.setLeft(helpAdd(curr.getLeft(), data));
        } else if (newNode.getData().compareTo(curr.getData()) > 0) { // data bigger than curr.data
    ...
    public T remove(T data) {
        ...
        AVLNode<T> returnNode = new AVLNode(-1);
        root = helpRemove(root, data, returnNode);
        size--;
        return returnNode.getData();
    }

    private AVLNode<T> helpRemove(AVLNode<T> curr, T data, AVLNode<T> returnNode) {
        AVLNode<T> dataNode = new AVLNode<>(data);
        if (curr == null) { // data not found case
            throw new NoSuchElementException("Data not found.");
        } else if (dataNode.getData().compareTo(curr.getData()) < 0) { // data smaller than curr.data
            curr.setLeft(helpRemove(curr.getLeft(), dataNode.getData(), returnNode));
        } else if (dataNode.getData().compareTo(curr.getData()) > 0) {
            curr.setRight(helpRemove(curr.getRight(), dataNode.getData(), returnNode));
        } else { // data found
            returnNode.setData(curr.getData());
            if (curr.getLeft() == null && curr.getRight() == null) { // no children
                ...
                AVLNode<T> successor = new AVLNode(-1);
                curr.setRight(getSuccessor(curr.getRight(), successor));
    ...
    private AVLNode<T> getSuccessor(AVLNode<T> curr, AVLNode<T> successor) {
        if (curr.getLeft() == null) {
            successor.setData(curr.getData());
            return curr.getRight();
            } else curr.setLeft(getSuccessor(curr.getLeft(), successor));
        curr = balance(curr);
        return curr;
    }
    ...
    private void updateHeightAndBF(AVLNode<T> currentNode) {
        ...
        if (currentNode.getLeft() == null) leftH = -1;
        else leftH = currentNode.getLeft().getHeight();
        if (currentNode.getRight() == null) rightH = -1;
        else rightH = currentNode.getRight().getHeight();

        if (leftH > rightH) currentNode.setHeight(leftH + 1);
        else currentNode.setHeight(rightH + 1);
